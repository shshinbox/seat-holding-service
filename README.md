...
콘서트 좌석을 선점하고, 사용자가 예약하기를 확정했을 때 선점 좌석 목록을 Kafka로 발행하는 서비스입니다. 좌석 페이지 진입 시점에 대기열 서버로부터 admission 토큰(JWT)을 받아오고, 이후 좌석 관련 API는 해당 토큰을 검증한 후에만 처리합니다.
...
좌석 페이지 진입 시 호출하는 `POST /schedules/{scheduleId}/entry` 엔드포인트는 대기열 서버의 admit 엔드포인트를 호출해 토큰을 받아 클라이언트에 반환합니다.


토큰은 HS256 서명된 JWT이며, 시크릿은 대기열 서버와 공유합니다.

- 서명, `exp` (만료)
- `type` 클레임이 `QUEUE_PASS`
- `sub` 클레임이 `X-Authenticated-User-Id` 헤더 값과 일치
- `scheduleId` 클레임이 URL path의 `{scheduleId}`와 일치

조건이 하나라도 어긋나면 `401 Unauthorized`를 반환합니다.

```json
{
  "message": "Queue pass token verification failed."
}
```
...
```text
POST /schedules/{scheduleId}/entry
  -> AuthenticatedUserArgumentResolver
      1. X-Authenticated-User-Id 헤더에서 userId 추출
  -> EntryController.entry
  -> QueueEntryService.enter
  -> WaitingQueueClient (WebClient)
      1. POST {waiting-queue.base-url}/internal/v1/queue/admit?scheduleId=...
      2. X-Authenticated-User-Id 헤더 그대로 forward
  -> API 응답 (token)
```

이 엔드포인트는 토큰 검증 대상이 아닙니다. 클라이언트는 응답으로 받은 토큰을 이후 좌석 API 호출에 사용합니다.

## 좌석 클릭 흐름

```text
POST /schedules/{scheduleId}/seats/{seatId}/holds
  -> QueuePassTokenFilter
      1. Authorization Bearer 토큰 추출
      2. JWT 서명/exp/type/sub/scheduleId 검증
  -> AuthenticatedUserArgumentResolver
      1. X-Authenticated-User-Id 헤더에서 userId 추출
...
좌석 페이지 첫 진입 시 호출합니다. 대기열 서버에서 admit 판정과 JWT를 받아 그대로 반환합니다.
...
좌석을 클릭하면 hold 또는 release 중 하나로 처리됩니다. 진입 시 발급받은 토큰을 `Authorization: Bearer`로 함께 전달합니다.
...
현재 사용자가 선점 중인 좌석 목록을 Kafka로 발행합니다. 진입 시 발급받은 토큰을 `Authorization: Bearer`로 함께 전달합니다.
